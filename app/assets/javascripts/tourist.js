(function() {
  ...
  */
  ...
  Tourist.Tip.Base = (function() {
    ...
    Base.prototype.skipButtonTemplate = _.template('<button class="btn btn-default btn-sm pull-right tour-next"><%= buttonLabel %></button>');

    Base.prototype.nextButtonTemplate = _.template('<button class="btn btn-primary btn-sm pull-right tour-next"><%= buttonLabel %></button>');

    Base.prototype.finalButtonTemplate = _.template('<button class="btn btn-primary btn-sm pull-right tour-next"><%= buttonLabel %></button>');

    Base.prototype.closeButtonTemplate = '<a class="btn btn-close tour-close" href="#"><i class="icon icon-remove"></i></a>';

    Base.prototype.okButtonTemplate = _.template('<button class="btn btn-sm tour-close btn-primary"><%= buttonLabel %></button>');
    ...
    */
    ...
    Base.prototype._setTarget = function(target, step) {
      ...
      if (target && step && step.highlightTarget && (typeof target === Object)) {
    ...
    Base.prototype._buildButtons = function(step) {
      ...
      buttons = '';
      if (step.okButton) {
        buttons += this.okButtonTemplate({buttonLabel: step.buttonLabel});
      }
      if (step.skipButton) {
        buttons += this.skipButtonTemplate({buttonLabel: step.buttonLabel});
      }
      if (step.nextButton) {
        buttons += step.final ? this.finalButtonTemplate({buttonLabel: step.buttonLabel}) : this.nextButtonTemplate({buttonLabel: step.buttonLabel});
      }
      return buttons;
    };
